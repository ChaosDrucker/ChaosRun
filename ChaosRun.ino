...
// ***********************************************************************
const byte LevelSpeed1[] =         { 165,160,160,155,150,145,140,135,130,125,120,115,110,105,100, 98, 96, 94, 92, 90, 88, 86, 84, 82, 80, 78, 76, 74, 72, 70}; 
...
void setup()
{
  ...
  if (SavingNeeded==111) { Brightness = GameBright[GameMode] * 20; SavingNeeded = 0;}
  else                   { Brightness = 5; SavingNeeded = 1;}
...
void copyLevel(byte lvl)
{ // Abhängig von "ValueLevel" die Werte in "LevelSpeed[] kopieren
  switch (lvl)
  { case 0: // LevelSpeed1
...
void writeEEPROM()
{ // Inhalt der Einstellungen in das EEPROM schreiben
  // byte       GameMode        = 2;                 // Eingestellte Spielvariante
  // byte       GameModeMax     = 2;                 // Wieviele Spielvarianten gibt es = max. 5 !
  ...
  data = 111;         EEPROM.write(address,data); address++;
  // GameMode
  data = GameMode;    EEPROM.write(address,data); address++;
  data = GameModeMax; EEPROM.write(address,data); address++;
  // GameLevel
  for (index=0; index<6; index++) { data=GameLevel[index]   ; EEPROM.write(address,data); address++; } 
  for (index=0; index<6; index++) { data=GameLevelMax[index]; EEPROM.write(address,data); address++; } 
  // GameBright
  for (index=0; index<6; index++) { data=GameBright[index]   ; EEPROM.write(address,data); address++; } 
  for (index=0; index<6; index++) { data=GameBrightMax[index]; EEPROM.write(address,data); address++; } 
  // Save-Anforderung zurücksetzen
  SavingNeeded = 0;
}

void readEEPROM()
{ // Inhalt der Einstellungen aus dem EEPROM lesen
  // val = EEPROM.read(addr) 
  byte address, data, index;
  address=0;
  // StatusByte lesen - 111 = gültige Daten vorhanden
  data = EEPROM.read(address); SavingNeeded = data; address++;
  if (SavingNeeded==111) 
  { // restliche Daten einlesen
    Serial.println("Es sind gültige Daten im EEPROM vorhanden !");
    // GameMode
    data = EEPROM.read(address); GameMode    = data; address++;
    data = EEPROM.read(address); GameModeMax = data; address++;
    // GameLevel
    for (index=0; index<6; index++) { data=EEPROM.read(address); GameLevel[index]   =data; address++; } 
    for (index=0; index<6; index++) { data=EEPROM.read(address); GameLevelMax[index]=data; address++; } 
    // GameBright
    for (index=0; index<6; index++) { data=EEPROM.read(address); GameBright[index]   =data; address++; } 
    for (index=0; index<6; index++) { data=EEPROM.read(address); GameBrightMax[index]=data; address++; } 
  }
}

void loop()
{  // Einstellunmgen "Spiel-unabhängig"	
    switch (GameState)
      { // Den Status im Spiel auswerten
        case 0: Serial.println("Loop : SetupGame");
                setupGame();
                break;
        case 1: Serial.println("Loop : InitGame");
                initGame();
                break;
	      case 2: Serial.println("Loop : RunGame");
                runGame();
                break;
        case 3: Serial.println("Loop : EndGame");
                endGame();
                break;
      }
}
...
{	Serial.println("Sub : SetupGame");
  Serial.print("GameMode : "); Serial.print(GameMode);Serial.print(" ConfigState : "); Serial.println(ConfigState);
  switch (ConfigState)
...
void setupKeys (byte t, byte lvl, byte lvlMax)
{ // t=typus (Welcher Knopf) l=lvl (Alter Wert)
  //unsigned long startZeit, aktZeit, diffZeit;
  TimeOut = 2000;
  byte taste;
  delay(100);
  Serial.print("SUB setupKeys : Relevanter Taster : ");Serial.print(taste);Serial.print(" Start-L : ");Serial.print(lvl);Serial.print(" Max : ");Serial.println(lvlMax);
  delay(100);
  Score[t] = lvl; 
  setScore(t+10);
  //taste = t+1; // 1 <= taste <= 4
  ...
  while (DiffZeit < TimeOut)
    { // Nach 2 Sekunden Inaktivität wird die Einstellung beendet
      digitalWrite(LED, HIGH);
      if (digitalRead(ButtonPin[t]) == LOW)
      { //  Taste gedrückt
          ...
          if (lvl > 5)      { lvl = 1; }
          Score[t] = lvl; setScore(t+10);
          if (t==2) { FastLED.setBrightness(lvl*20); FastLED.show(); } // Brightness DIREKT anzeigen
          delay(100);
          StartZeit=millis();
       }
       delay(100);
   ...
   Serial.print("SUB setupKeys - Taste : ");Serial.print(taste);Serial.print(" Aktueller L : ");Serial.println(lvl);
   delay(500);
   KeyValue=lvl;
...
{ Serial.print("Sub : SetupUniversal Typus : "); Serial.println(typus);
  byte lvl;
  byte player;
  ...
  { case 1: // GameMode
            ...
            lvl    = GameMode;
            player = typus - 1;
            Serial.print("SUB setupUniversal : Relevanter Taster : ");Serial.print(typus-1);Serial.print(" Start-L : ");Serial.println(lvl);
            delay(100);
    ...
    case 3: // Brightness
			...
			if (lvl!=KeyValue) { SavingNeeded=1; }
            lvl = KeyValue;
            FastLED.setBrightness(lvl*20); FastLED.show();
...
{ Serial.print("Sub : RunGame - GameMode ");Serial.println(GameMode);
  GameRunning = true;
...
{ Serial.println("GameState 2 -> runGame Spiel ChaosRun");
  long DiffZeit = ( LevelSpeed[Level-1] ) * 2 ; // byte -> integer (long)
  long randdir = 0;
  ...
  // ===
  while (GameRunning) {
    DiffZeit = ( LevelSpeed[Level-1] ) * 2 ; // byte -> integer (long)
    if ( millis() > (AktZeit + DiffZeit) ){
      AktZeit = millis();
      Serial.print("Level : "); Serial.print(Level); Serial.print( " DiffZeit : "); Serial.println(DiffZeit);
      // Target wiederholen
      showTarget(1);
      // Runner setzen
      showRunner(0);
    ...
    { // Der Spieler, der gedrückt hat, steht in "PressedButton" (0-3)
      ...
      PlayerBtns[4]=false;
      Serial.print("Spieler "); Serial.print(PressedButton + 1); Serial.print(" hat gedrückt und ");
      if (PlayerActive[PressedButton] == 1) {
        Serial.println("wird gewertet");
        Serial.print("TargetPos : ");Serial.print(TargetPosAct);Serial.print(" RunnerPos : ");Serial.print(RunnerPosAct);Serial.print( "RunnerDir : ");Serial.println(RunnerDirection);
        //   Ist Position WINNER oder LOSER ?
        //   CALL Winner() oder CALL Loser()  
        if (RunnerDirection==0) {
          if (TargetWidth==3) {
            if (RunnerPosAct >= TargetPosAct - 1 && RunnerPosAct <= TargetPosAct + 1) { winner(PressedButton);  }
            else                                                                      { loser(PressedButton); }
          }
          else { if (RunnerPosAct == TargetPosAct || RunnerPosAct == TargetPosAct ) { winner(PressedButton); }
                 else                                                                  { loser(PressedButton); }
          }
        }  
      ...
      else  { Serial.println("wird nicht gewertet");
      }
    memset(PlayerBtns, 0, sizeof(PlayerBtns));
    //   Spielende erreicht ?
...
{ Serial.println("GameState 2 -> runGame Spiel Reaction");
  ...
  { //Start des Contest
    ...
    gameLevel++;
    gameCounter++;
    runDelayR = 0; runDelayG = 0;
    for (x=0;x<33;x++) { LEDRing[x] = UrRing[x]; } 
    // An x=8=GRÜN 
    for (x=0;x<9;x++) {ringLeds[x].setRGB(64,64,64); }
    actPosG = 2;
    for (x=29;x<33;x++) {ringLeds[x].setRGB(64,64,64); }
    ringLeds[actPosG].setRGB(0,255,0); LEDRing[actPosG]=12;
    targetG1 =  1; oldPosG =  2; targetG2 =  3;
    LEDRing[targetG1] = 22; LEDRing[targetG2] = 22;
    ringLeds[targetG1].setRGB(0,0,32); ringLeds[targetG2].setRGB(0,0,32);
    targetsG=1;
    // An x=24=ROT
    for (x=12;x<25;x++) {ringLeds[x].setRGB(64,64,64); }
    actPosR = 18;
    ringLeds[actPosR].setRGB(255,0,0); LEDRing[actPosR]=10;
    targetR1 = 17; oldPosR = 24; targetR2 = 19; 
    LEDRing[targetR1] = 20; LEDRing[targetR2] = 20;
    ringLeds[targetR1].setRGB(0,0,32); ringLeds[targetR2].setRGB(0,0,32);
    targetsR=1;
    FastLED.show();
    ...
    // let the Runners run ...
    Serial.print("Game-Level : ");Serial.println(gameLevel);
    GameRunning = true;
    while (GameRunning) 
    { // Start des Games
      ...
      { // Serial.println("Spiel läuft : Neue Position aufbauen");
        AktZeit = millis();
	      if ( (runDelayG == 0) )
        { // Alte GRÜN-Position löschen
        if (LEDRing[oldPosG] != 0) { if      ( LEDRing[oldPosG] == 10 ) { ringLeds[oldPosG].setRGB(255,0,0); }
                                     else if ( LEDRing[oldPosG] == 12 ) { ringLeds[oldPosG].setRGB(0,255,0); }
                                     else if ( LEDRing[oldPosG] == 15 ) { ringLeds[oldPosG].setRGB(64,64,64); }
                                     else if ( LEDRing[oldPosG] == 20 ) { ringLeds[oldPosG].setRGB(0,0,32); }
                                     else if ( LEDRing[oldPosG] == 22 ) { ringLeds[oldPosG].setRGB(0,0,32); }
                                   }
        else                       { ringLeds[oldPosG].setRGB(0,0,0); }
        // Neue GRÜN-Position ermitteln und setzen
        // Serial.print("actPosG = "); Serial.println(actPosG);
        if (dirG==0) { actPosG++; if      ((actPosG >  8) && (actPosG < 28)) { dirG=1; actPosG=  7;} 
                                  else if  (actPosG > 32)                    { dirG=0; actPosG=  0;} 
                     } 
                else { if (actPosG > 0) { actPosG--; if ((actPosG < 29) && (actPosG > 10)) { dirG=0; actPosG= 30;} }
                       else                                                                { dirG=1; actPosG= 32;}                                            
                     } 
        ringLeds[actPosG].setRGB(0,32,0);
        oldPosG = actPosG;
      }
        else
        { if ( (runDelayG==5) || (runDelayG==3) || (runDelayG==1) ) { ringLeds[actPosG].setRGB(0,0,0); }
          else                                                       { ringLeds[actPosG].setRGB(0,255,0); }   
          Serial.print("Pause für Grün : "); Serial.println(runDelayG);
 		      if (runDelayG > 0)runDelayG--;
        }   
        // --------------------------------------
	      if ( (runDelayR == 0))
	      { // Alte ROT-Position löschen
        if (LEDRing[oldPosR] != 0) { if      ( LEDRing[oldPosR] == 10 ) { ringLeds[oldPosR].setRGB(255,0,0); }
                                     else if ( LEDRing[oldPosR] == 12 ) { ringLeds[oldPosR].setRGB(0,255,0); }
                                     else if ( LEDRing[oldPosR] == 15 ) { ringLeds[oldPosR].setRGB(64,64,64); }
                                     else if ( LEDRing[oldPosR] == 20 ) { ringLeds[oldPosR].setRGB(0,0,32); }
                                     else if ( LEDRing[oldPosR] == 22 ) { ringLeds[oldPosR].setRGB(0,0,32); }
                                   }  
        else                       { ringLeds[oldPosR].setRGB(0,0,0); }
        // Neue ROT-Position ermitteln und setzen
        if (dirR==0) { actPosR++; if (actPosR>24) { dirR=1; actPosR=23;} } 
                else { actPosR--; if (actPosR<12) { dirR=0; actPosR=13;} } 
        ringLeds[actPosR].setRGB(32,0,0);
        oldPosR = actPosR;
		    if (runDelayR > 0) runDelayR--;
      }
        else
        { if ( (runDelayR==5) || (runDelayR==3) || (runDelayR==1) ) { ringLeds[actPosR].setRGB(0,0,0); }
          else                                                       { ringLeds[actPosR].setRGB(255,0,0); }   
           Serial.print("Pause für Rot : "); Serial.println(runDelayR);
 		       if (runDelayR > 0) runDelayR--;
        } 
	      FastLED.show(); 
        debounceR = 0; debounceG = 0;
      }  
      else // Tasten-Handling
      { // Serial.println("Spiel läuft : Tasten abfragen");
        btn1Bounce.update();
          if(btn1Bounce.fell())
            { // ROT : Beide Spieler gleichberechtigt abfragen
            Serial.print("ROT hat gedrückt an Position ");Serial.print(actPosR);Serial.print(" LED: ");Serial.print(LEDRing[actPosR]);
            if (LEDRing[actPosR]==20) 
            { // Target getroffen
  			      Serial.println(" Winner !");
              LEDRing[actPosR]=10; // Target erobert -> ROT
              if (actPosR>18) { newTarget=actPosR+1; if (dirR==0) actPosR++;} 
              else            { newTarget=actPosR-1; if (dirR==1) actPosR--;} 
              if (LEDRing[newTarget] != 0) { LEDRing[newTarget]=20; ringLeds[newTarget].setRGB(0,0,32); }
              targetsR++;
              //runDelayR = 1;
            }
            else   
            ...
            PlayerBtns[0]=false; debounceR = 1;
          }
        btn3Bounce.update();  
        if(btn3Bounce.fell())
          { // GRÜN : Beide Spieler gleichberechtigt abfragen
            Serial.print("GRÜN hat gedrückt an Position ");Serial.print(actPosG);Serial.print(" LED: ");Serial.print(LEDRing[actPosG]);
            if (LEDRing[actPosG]==22) 
            { // Target getroffen
              Serial.println(" Winner !");
              LEDRing[actPosG]=12; // Target erobert -> GRÜN
              if      ( ( actPosG > 2   ) && ( actPosG < 28  ) ) { newTarget = actPosG+1; if (dirG==0) actPosG++;  }  // Target PLUS
              else if ( ( actPosG < 2   ) && ( actPosG > 0   ) ) { newTarget = actPosG-1; if (dirG==1) actPosG--;  }  // Target MINUS
              else if ( ( actPosG >= 28 ) && ( actPosG <= 32 ) ) { newTarget = actPosG-1; if (dirG==1) actPosG--;  }  // Target MINUS
              else if (   actPosG == 0                         ) { newTarget = 32;        if (dirG==1) actPosG=31; }  // Target SET
              Serial.print("New Target = ");Serial.println(newTarget);
              if (LEDRing[newTarget] != 0) { LEDRing[newTarget]=22;  ringLeds[newTarget].setRGB(0,0,32); }
              targetsG++;  
            ...
            PlayerBtns[2]=false; debounceG = 1;
          }
        } // Ende Tastenauswertung
        PlayerBtns[4]=false; PlayerBtns[2]=false; PlayerBtns[0]=false;
        if (targetsR == 13) { winner(0); }
        if (targetsG == 13) { winner(2); }
        if ( (targetsR == 13) || (targetsG == 13) ) { GameRunning = false; memset(LEDRing,0, sizeof(LEDRing)); targetsR = 1; targetsG = 1; }
      //} // end of Tasten-Handling
    } // end of GameRunning
    if ( (Score[0]==gamesToWin) || (Score[2]==gamesToWin) ) { ContestRunning=false; GameState = 3; }
...
{  
  ...
  if (digitalRead(Button1Pin) == LOW) { setPlayerButtonsState (0, true); setPlayerButtonsState(4,true); }
  if (digitalRead(Button3Pin) == LOW) { setPlayerButtonsState (2, true); setPlayerButtonsState(4,true); }
}
...
{ // Siegerehrung
  ...
  for ( int i = 0; i < sizeof(Score); i++ ) {
    Serial.print(" #");Serial.print(i);Serial.print(" : ");Serial.print(Score[i]);
    delay(50);
    if ( Score[i] > maxVal ) {
      maxVal = Score[i];
      maxIndex = i;
      maxVal2 = 0; maxIndex2 = 0;
    }
  ...
  for (byte flash = 0; flash < maxflash; flash++) {
    ...
    level=maxIndex2+10;
    if (maxIndex2 > 0) { for (byte i = 0; i < anzahl; i++) { scoreLeds[start + i].setRGB(R[level],G[level],B[level]); } }
    FastLED.show();
    delay(500);
  }  
...
void DetectSettings() 
{  // Hier wird auf Tastendrücke gewartet, die die Einstellungen (Settings) beeinflussen
   ...
   for (byte i=0;  i<5;  i++) {scoreLeds[i].setRGB(255,  0,  0); delay(50);}
   for (byte i=5;  i<10; i++) {scoreLeds[i].setRGB(255,255,  0); delay(50);}
   for (byte i=10; i<15; i++) {scoreLeds[i].setRGB(  0,255,  0); delay(50);}
   for (byte i=15; i<20; i++) {scoreLeds[i].setRGB(  0,  0,255); delay(50);}
   FastLED.show();
   ...
   while ( (digitalRead(Button1Pin)==HIGH) && (digitalRead(Button2Pin)==HIGH) && (digitalRead(Button3Pin)==HIGH) && (digitalRead(Button4Pin)==HIGH) ) 
    { idleAnimation(FadeInterval); delay(50); }
   ...
   AktZeit = millis();
     ...
     { StartZeit = millis();
       ...
       {  // Rot : Taster für GameMode
          AktZeit = millis();
          ConfigState = 1;
		  buttonPressed = 1;
       }
       else if (digitalRead(Button2Pin)==LOW) 
       {  // Gelb : Taster für GameLevel
          AktZeit = millis();
          ConfigState = 2;
	      buttonPressed = 1;
       }
...
void DetectPlayers() 
{ // Alte Programmierung zum Testen angepasst an neue Struktur und neue Variablen
  ...
  scoreLeds[2].setRGB (255,   0,    0); delay(50);
  scoreLeds[7].setRGB (255, 255,    0); delay(50);
  scoreLeds[12].setRGB(  0, 255,    0); delay(50);
  scoreLeds[17].setRGB(  0,   0,  255); delay(50);
  FastLED.show();
  // Auf einen ersten Tastendruck warten
  AktZeit = millis();
  StartZeit = millis(); 
  while ( ( (AktZeit - StartZeit) < 3000) || ( (PlayerActive[0]==0) && (PlayerActive[1]==0) && (PlayerActive[2]==0) && (PlayerActive[3]==0) ) ) {
    idleAnimation(FadeInterval);
    if (((digitalRead(Button1Pin)==LOW) ) && (PlayerActive[0]==0) ) { PlayerActive[0] = 1; for (byte k= 1; k< 4; k++) { scoreLeds[k].setRGB(255,   0,   0); delay(50);} FastLED.show(); StartZeit=millis(); }
    if (((digitalRead(Button2Pin)==LOW) ) && (PlayerActive[1]==0) ) { PlayerActive[1] = 1; for (byte k= 6; k< 9; k++) { scoreLeds[k].setRGB(255, 255,   0); delay(50);} FastLED.show(); StartZeit=millis(); }
    if (((digitalRead(Button3Pin)==LOW) ) && (PlayerActive[2]==0) ) { PlayerActive[2] = 1; for (byte k=11; k<14; k++) { scoreLeds[k].setRGB(0  , 255,   0); delay(50);} FastLED.show(); StartZeit=millis(); }
    if (((digitalRead(Button4Pin)==LOW) ) && (PlayerActive[3]==0) ) { PlayerActive[3] = 1; for (byte k=16; k<19; k++) { scoreLeds[k].setRGB(0  ,   0, 255); delay(50);} FastLED.show(); StartZeit=millis(); }
  ...
  FastLED.show();
  delay(1000);
  if (PlayerActive[0]==1) { for (byte k= 0; k <5; k++) { scoreLeds[k].setRGB(255,   0,   0); delay(50); FastLED.show(); } }
  if (PlayerActive[1]==1) { for (byte k= 5; k<10; k++) { scoreLeds[k].setRGB(255, 255,   0); delay(50); FastLED.show(); } }
  if (PlayerActive[2]==1) { for (byte k=10; k<15; k++) { scoreLeds[k].setRGB(0  , 255,   0); delay(50); FastLED.show(); } }
  if (PlayerActive[3]==1) { for (byte k=15; k<20; k++) { scoreLeds[k].setRGB(0  ,   0, 255); delay(50); FastLED.show(); } }
  ...
  while ( ( ( ( (PlayerActive[0]==1) && (digitalRead(Button1Pin)==LOW)) || (PlayerActive[0]==0) )  &&
            ( ( (PlayerActive[1]==1) && (digitalRead(Button2Pin)==LOW)) || (PlayerActive[1]==0) )  && 
            ( ( (PlayerActive[2]==1) && (digitalRead(Button3Pin)==LOW)) || (PlayerActive[2]==0) )  &&
            ( ( (PlayerActive[3]==1) && (digitalRead(Button4Pin)==LOW)) || (PlayerActive[3]==0) )  ) == false ) { delay(50); }
...
void winner(byte player) 
{ // Punkte hinzufügen und neuen Score anzeigen
  byte Punkte;
  byte r,g,b;
  r = R[player+10];
  g = G[player+10];
  b = B[player+10];
  Punkte = Score[player];  Punkte++; Score[player]=Punkte;
  // "La Ola"
  for (byte i = 0; i < 3; i++) {
  ...
  if (GameMode==1) 
  {
  if (Level>MaxLevel) { delay(100); } else { showTarget(0); }
  delay(1000);
  // Verzögerung, damit alle das neue Target sehen können
  }
}

void loser(byte player) 
{ // Punkte abziehen und neuen Score anzeigen
...
void setScore(byte player) 
  { // Routine, um die Scores jedes Spieler anhand der Punkte komplett zu setzen
  byte Punkte;
  byte Start, Level, Anzahl;
  if (player>9)
  { // Setzen des kompletten Scores in Spielerfarbe
    Start=Score1stLED[player-10]; 
    Punkte = Score[player-10];
    Serial.print("SetScore Spieler "); Serial.print(player); Serial.print(" Punkte "); Serial.println(Punkte);
    delay(10);
    // "Alte Punkte" löschen
...
{	
  ...
  memset(PlayerBtns, 0, sizeof(PlayerBtns));
	// poll Buttons
  if (digitalRead(Button1Pin) == LOW) { setPlayerButtonsState (0, true); setPlayerButtonsState(4,true); PressedButton=1; } else setPlayerButtonsState (0, false);
  if (digitalRead(Button2Pin) == LOW) { setPlayerButtonsState (1, true); setPlayerButtonsState(4,true); PressedButton=2; } else setPlayerButtonsState (1, false);
  if (digitalRead(Button3Pin) == LOW) { setPlayerButtonsState (2, true); setPlayerButtonsState(4,true); PressedButton=3; } else setPlayerButtonsState (2, false);
  if (digitalRead(Button4Pin) == LOW) { setPlayerButtonsState (3, true); setPlayerButtonsState(4,true); PressedButton=4; } else setPlayerButtonsState (3, false);
	// print Debug 
...
{ byte ledDiff;
  ...
  { case 0:  // Neue Position ermitteln und anzeigen
     ...
     if (Level < SwitchTargetPrecisionAtLevel) { TargetWidth = 3; ledDiff = 7; }
     else                                      { TargetWidth = 1; ledDiff = 5; }
	   // Target soll nicht näher als 5 LEDs an RunnerPosAct gebaut werden    
     TargetPosAct = random(2,RING_LEDS_NUM-2);
       while ( (TargetPosAct-RunnerPosAct < ledDiff) && (RunnerPosAct-TargetPosAct < ledDiff) )
         { TargetPosAct = random(2,RING_LEDS_NUM-2); }
	     RunnerDirection  = random(0,2);
       Serial.print("Neues Target ! TargetPos : "); Serial.print(TargetPosAct); Serial.print(" RunnerPos : "); Serial.print(RunnerPosAct);Serial.print("  RunnerDir : "); Serial.println(RunnerDirection);
	     // Target anzeigen
       if (TargetWidth==3) {
...
{ RunnerPosOld = RunnerPosAct;
  ...
  { case 0:
    ...
    { //Serial.print("RunnerDirection = "); Serial.print(RunnerDirection);
...
void clearScoreLeds(byte player, byte modus) 
{ // Score-LEDs löschen in unterschiedlichen Modi
  Serial.print("ClearScore bei Spieler "); Serial.println(player);
  delay(100);
  byte firstLED;
  byte range;
  switch (player)
  ...
  Serial.print("Lösche Score von "); Serial.print(firstLED); Serial.print(" bis "); Serial.println(firstLED+range);
  delay(100);
  if (modus == 0) 
  { // Schlagartig löschen
...
void clearRingLeds(byte mode) 
{ // Ring-LEDs löschen in unterschiedlichen Modi
  byte firstLED = 0;
  byte range    = RING_LEDS_NUM;
  switch (mode)
  { case 0:
      // Alle Ring-LED auf einmal löschen
      for (byte i=firstLED; i<range; i++) 
	      { // Alle LED "im dunklen" löschen
          ringLeds[i].setRGB(  0,   0,  0);
    	  }  
      FastLED.show();
      break;
